...
var register  = function (match, l, callback) {
   ...
   */
  emitter.emit('reg-start-' + l, Object.keys(match.bin).length);
  ...
  var tar = untilde(rc.dir.tmp + '/' + long + '.tar');

  var j = Object.keys(match.bin).length - 1;
  emitter.on('reg-data-' + j, function () {
    /**
  ...
  for (var bin in match.bin) {
     ...
     * @type  {object}
     * @param {string} location Where the file is registered.
     * @see   RucheUtil.register
     */
    emitter.emit('reg-data-' + i, file);
  }
