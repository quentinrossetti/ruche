...
var install =  function (packages) {
  ...
  var repository = path.resolve(__dirname + '../../../package/');
  ...
  for (var i = packages.length - 1; i >= 0; i--) {
    ...
    fs.readFile(file, { encoding: 'utf8'}, function (err, data) {
      ...
      if (!match) {
        ...
        emitter.on('install-' + match.package + '-file', function (data) {
          var script = '../../package/' + match.package + '/script/' + match.script;
          debug('Use installation script: ' + match.script);
          require(script)(match);
        });
        
        debug('Installation of %s started', long);
        var dl = match.package + '/' + long + '.' + package.format;
        var destFile = path.resolve(__dirname, '../../tmp/' + dl);
        var destDir = path.dirname(destFile);
        
        // This download will emit a RucheEmitter#install-package-file 
        // event on completion. 
        download(match.package, match.link, destFile, match.options);
      }
    });
  }
  // End of process
