...
describe('ruche:util:format', function () {
  ...
  it('`software-0.0.1` should return the name and version', function () {
    var software = u.format('git-0.0.1');
    expect(software.name).to.eql('git');
  ...
  it('`software-0.0.1beta` should return the name and version', function () {
    var software = u.format('git-0.0.1beta');
    expect(software.name).to.eql('git');
    expect(software.version).to.eql('0.0.1beta');
    expect(software.options).to.eql(null);
    expect(software.platform).to.eql(null);
  });

  it('`soft2-24.1-win64` should return name, version, platform', function () {
    var software = u.format('soft2-24.1-win64');
  ...
  it('`s2-2.1-ssl_v6-win64` should return a complete spec', function () {
    var software = u.format('s2-2.1-ssl_v6-win64');
    expect(software.name).to.eql('s2');
    expect(software.version).to.eql('2.1');
    expect(software.options).to.eql([ 'ssl', 'v6' ]);
    expect(software.platform).to.eql('win64');
  });

  it('`test?` should fail', function () {
    var software = u.format('test?');
    expect(software).to.eql(null);
  });
